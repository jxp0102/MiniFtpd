...
static void do_cdup(session_t *sess);//上一层
static void do_quit(session_t *sess);//传输中止
static void do_port(session_t *sess);
static void do_pasv(session_t *sess);
static void do_type(session_t *sess);
static void do_stru(session_t *sess);
static void do_mode(session_t *sess);
static void do_retr(session_t *sess);//下载文件（包括续载）
static void do_stor(session_t *sess);//上传文件（客户端向服务器传）
static void do_appe(session_t *sess);
static void do_list(session_t *sess);
static void do_nlst(session_t *sess);//n个目录列表
static void do_rest(session_t *sess);//断点续传
static void do_abor(session_t *sess);
static void do_pwd(session_t *sess);//当前目录
static void do_mkd(session_t *sess);//创建目录
static void do_rmd(session_t *sess);//删除目录
static void do_dele(session_t *sess);//删除
static void do_rnfr(session_t *sess);//从一个名字更改
static void do_rnto(session_t *sess);//到另一个名字 这是一对命令，更改名字
static void do_site(session_t *sess);//位置信息
static void do_syst(session_t *sess);
static void do_feat(session_t *sess);
static void do_size(session_t *sess);//大小
static void do_stat(session_t *sess);
static void do_noop(session_t *sess);//没有操作
...
void limit_rate(session_t *sess,int transfered_bytes,int is_upload);
{
	...
	double pause_time;  //计算睡眠的时间
	pause_time=(rate_ratio-(double)1)*elapsed;

	nano_sleep(pause_time); //睡眠时间的操作

	sess->bw_transfer_start_sec=get_time_sec(); //把下一次传输时间更新为当前时间
...
{
	/*
	向nobody发送PRIV_SOCK_GET_DATA_SOCK    1
	向nobody发送一个整数port               4
	向nobody发送一个字符串                 不定长
	*/
...
{
	//PASS 123456
	struct passwd *pw=getpwuid(sess->uid);  //pw指向的是所获取到用户id   passwd 里面的内容包括pw_name
	if(pw==NULL)
	{
		//用户不存在
		ftp_reply(sess,FTP_LOGINERR,"Login incorrect.");
...
{
	char text[1024]={0}; //获取路径
	char dir[1024+1]={0};//目录
	getcwd(dir,1024);
	sprintf(text,"\"%s\"",dir);
